...
前処理の手順
1. どんな予測モデルを作成するのか明確にするために、種類列のデータにどんな値があるか確認する。
	- 正解データのそれぞれの件数に大きな偏りがある。極端に偏りがあると、法則などを考えずに割合の大きい方と答えを出してしまう。
2. 収集したデータに欠損値があるとデータ分析ができないため、欠損値があるかないか確認する。
	- 大半のデータが欠損値の場合、他の値で穴埋めしても信頼性が低いため処理をせず特徴量に加えないようにする。
3. 前処理した学習用のデータを特徴量と正解データに分割する。
4. 特徴量と正解データを訓練データとテストデータに分割する。
...
前処理の手順
1. どんな予測モデルを作成するのか明確にするために、種類列のデータにどんな値があるか確認する。
2. 学習に使いたいデータが文字列のため、ダミー変数化する。
3. モデルのチューニング時にテストデータに都合がよいようにチューニングしないように、学習用データを訓練データ、検証データ、テストデータの3つに分割する。
	- いきなり3つに分割せず、「訓練データ、検証データ」と「テストデータ」に分割して、「訓練データ、検証データ」を全データと仮定して、前処理・学習・評価をする。それから、「テストデータ」で最終的な予測性能を評価する。
	- 「ダミー変数化」は、対象となるデータによって処理が変わるため分割前にする必要がある。（分割時にあるグループにデータが入らないことを想定している。）
4. 「訓練データ、検証データ」に欠損値があるとデータ分析ができないため、欠損値があるかないか確認する。
5. 全体の傾向からかけ離れたデータ（外れ値）があると作成したモデルの性能が上がりにくくなるため、外れ値が含まれているか確認する。
	- データの種類が多いため、特徴量に使いたいデータにだけを処理する。
	- 外れ値が多い場合ときにキレイにしすぎるとテストデータに外れ値があったときに逆にさがるため、ある程度は残してもOK。
6. 使用する特徴量を絞り込むために、相関係数を調べる。
	- 特徴量と正解データの相関係数が大きいほど、特徴量が予測に大きな影響を与える。
	- 外れ値があると相関係数が0に近づいてしまうため、外れ値を除いてから実施する。
7. 前処理した「訓練データ、検証データ」から特徴量と正解データを取り出す。
8. 特徴量と正解データを訓練データと検証データに分割する。
9. それぞれの特徴量で値を1増加させる労力が異なる（各特徴量の分布が大きく異なる）とき、それぞれの特徴量の平均値とばらつきを統一させるために標準化する必要がある。
